...
<header class="bg-light2" x-data="{ isOpen: false }">
    <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
      ...
      <a href={{ route("home") }} class="-m-1.5 p-1.5">
        <span class="sr-only">{{ __('layout.nav.logo') }}</span>
        <img class="h-8 w-auto" src="/logo.svg" alt="logo">
      </a>
      <div class="flex flex-1 justify-end">
        @auth
            <a href="{{ route('dashboard') }}" class="text-sm/6 font-semibold text-gray-900">{{ __('layout.nav.dashboard') }}</a>
        @else
            <a href="{{ route('login') }}" class="text-sm/6 font-semibold text-gray-900">
              {{ __('layout.nav.login') }} <span aria-hidden="true">&rarr;</span>
            </a>
        @endauth
      </div>
    </nav>
        ...
        <div class="flex items-center justify-between">
          <div class="flex flex-1">
            <button type="button" class="-m-2.5 rounded-md p-2.5 text-gray-700" @click="isOpen = false">
              <span class="sr-only">{{ __('layout.nav.close') }}</span>
              <svg class="size-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon">
